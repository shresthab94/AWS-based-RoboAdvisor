...
def build_validation_result(is_valid, violated_slot,  
    """
...
def recommend_portfolio(intent_request):
    ...
    if age_check < 21 and age_check > 65:
            return build_validation_result(
                False, "age",
                    "The service is only for people above 21 and have not reached the retirement age of 65 ")
                ,
    if investment_amount< 5000:
        return build_validation_result(False,"investment_amount","Minimum investable amount is $5,0000.")
    return build_validation_result(True, None, None)
    
    if risk_level == "None":
        return build_validation_result(True,risk_level,“100% bonds (AGG) 0% equities (SPY)”)
    if risk_level == "Low":
        return build_validation_result(True,risk_level,“60% bonds (AGG), 40% equities (SPY)”)
    if risk_level == "Medium":
        return build_validation_result(True,risk_level,““40% bonds (AGG), 60% equities (SPY)”)
    if risk_level == "High":
        return build_validation_result(True,risk_level,“20% bonds (AGG), 80% equities (SPY)”)
...
def dispatch(intent_request):
    ...
    if intent_name == "recommendPortfolio":
        return recommend_portfolioz(intent_request)
